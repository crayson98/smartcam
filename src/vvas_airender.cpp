...
static const unsigned int CHANNELS = 3/2;
...
void writeRawRGBImage(const char *filename, uint8_t image[1920*1080]) {
    ...
    if (fp != NULL) {
        ...
        g_print ("nem sikerült képet írni\n");
    ...
    if (frameinfo->inframe->props.fmt == VVAS_VFMT_Y_UV8_420) {
      ...
      for (int i = 0; i < prediction->bbox.height; i++) {
          for (int j = 0; j < prediction->bbox.width; j++) {
                  conv.sptr[i * prediction->bbox.width + j] = frameinfo->lumaImg.data[start_index + i * 1920 + j + 128*i];
      ...
      for (int k = 0; k < prediction->bbox.height; k++) {
          for (int l = 0; l < prediction->bbox.width; l++) {
                  frameinfo->lumaImg.data[start_index + k * 1920 + l + 128*k] = conv.rptr[k * prediction->bbox.width + l];
          }
      }

      /*for (int k = 0; k < 200; k++) {
          for (int l = 0; l < 1920; l++) {
                  frameinfo->lumaImg.data[(200+k) * 1920 + l + 1000] = conv.rptr[k * 1920 + l];  
          }
      }*/
